...
def pca_plot(eof_dict,
    ...
    cbax_delta = f.add_axes([cb_delta_x0, 
                             cb_delta_y0, 
                             cb_delta_xlen, 
                             cb_delta_ylen])
    ...
    cmap55 = cmap_delta(0.01)
    cmap50 = cmap_delta(0.05)   #blue
    cmap45 = cmap_delta(0.1)
    cmap40 = cmap_delta(0.15)
    cmap35 = cmap_delta(0.2)
    cmap30 = cmap_delta(0.25)
    cmap25 = cmap_delta(0.3)
    cmap20 = cmap_delta(0.325)
    cmap10 = cmap_delta(0.4)
    cmap5 = cmap_delta(0.475)
    cmap0 = col_zero
    cmap_5 = cmap_delta(0.525)
    cmap_10 = cmap_delta(0.6)
    cmap_20 = cmap_delta(0.625)
    cmap_25 = cmap_delta(0.7)
    cmap_30 = cmap_delta(0.75)
    cmap_35 = cmap_delta(0.8)
    cmap_40 = cmap_delta(0.85)
    cmap_45 = cmap_delta(0.9)
    cmap_50 = cmap_delta(0.95)  #red
    cmap_55 = cmap_delta(0.99)
    
    colors_delta = [cmap_55,
                    cmap_50,
                    cmap_45,
                    cmap_35,
                    cmap_30,
    ...
    # declare list of colors for discrete colormap of colorbar
    cmap_list_delta = mpl.colors.ListedColormap(colors_delta,N=len(colors_delta))
    
    # colorbar args
    start = 0.006
    inc = start/8
    values_delta = [-1*start,
                    -1*start+inc,
                    -1*start+inc*2,
                    -1*start+inc*3,
                    -1*start+inc*4,
    ...
    tick_labels_delta = [str(round(-1*start,5)),
                         str(round(-1*start+inc,5)),
                         str(round(-1*start+inc*2,5)),
                         str(round(-1*start+inc*3,5)),
                         str(round(-1*start+inc*4,5)),
                         str(round(-1*start+inc*5,5)),
                         str(round(-1*start+inc*6,5)),
                         str(round(-1*start+inc*7,5)),
                         str(0),
                         str(round(start-inc*7,5)),
                         str(round(start-inc*6,5)),
                         str(round(start-inc*5,5)),
                         str(round(start-inc*4,5)),
                         str(round(start-inc*3,5)),
                         str(round(start-inc*2,5)),
                         str(round(start-inc,5)),
                         str(round(start,5))]
    
    norm_delta = mpl.colors.BoundaryNorm(values_delta,cmap_list_delta.N)
    ...
    # plot time series
    for exp in ['historical','hist-noLu']:
        data = pcs['cru'][exp] - pcs['cru'][exp].mean(dim='time')
        data.plot(ax=ax1,
                  add_legend=False,
                  color=cols[exp],
                  linestyle=ls_types['pc'],
                  linewidth=3,
                  label='MMM PC1 ' + exp)
        for obs in obs_types:
            data = pseudo_pcs[obs][exp] - pseudo_pcs[obs][exp].mean(dim='time')
            data.plot(ax=ax1,
                      add_legend=False,
                      color=cols[exp],
            ...
            if obs == 'cru':
                if i == 0:
                    ax.set_title('Obs EOF loading',
                                 ...
                                 fontsize=title_font)
                if i == 1:
                    ax.set_title('Δ EOF (hist - histnolu)',
                                 fontweight='bold',
                                 loc='center',
                                 fontsize=title_font)
                if i == 2:
                    ax.set_title('Δ EOF bias (hist - histnolu, bias = obs - mmm)',
                                 fontweight='bold',
                                 loc='center',
                                 fontsize=title_font)
            ax.set_title(letters[count],
    ...
    # delta cb
    cb_delta = mpl.colorbar.ColorbarBase(ax=cbax_delta, 
                                         cmap=cmap_list_delta,
                                         norm=norm_delta,
                                         spacing='uniform',
                                         orientation='horizontal',
                                         extend='neither',
                                         ticks=tick_locs_delta,
                                         drawedges=False)
    cb_delta.ax.xaxis.set_label_position('top');
    cb_delta.ax.tick_params(labelcolor=col_cbticlbl,
                            labelsize=tick_font,
                            color=col_cbtic,
                            length=cb_ticlen,
                            width=cb_ticwid,
                            direction='out'); 
    cb_delta.ax.set_xticklabels(tick_labels_delta,
                                rotation=45)
    cb_delta.outline.set_edgecolor(col_cbedg)
    cb_delta.outline.set_linewidth(cb_edgthic)
...
def pca_plot_luh2(eof_dict,
    ...
    ax2 = f.add_subplot(gs2[0],projection=ccrs.PlateCarree()) # lu eof
    ax3 = f.add_subplot(gs2[1],projection=ccrs.PlateCarree()) # luh2 defor eof
    ax4 = f.add_subplot(gs2[2],projection=ccrs.PlateCarree()) # luh2 crops eof
    
    map_axes = {}
    map_axes['lu'] = ax2
    map_axes['luh2_defor'] = ax2
    map_axes['luh2_crops'] = ax2
    ...
    for lc in landcover_types:
        #eof_dict[obs][lc]
        data = eof_dict['berkley_earth'][lc]
        for i,ax in enumerate(map_axes[obs]):
            count += 1
            if i == 0:
                data[i].plot(ax=ax,
